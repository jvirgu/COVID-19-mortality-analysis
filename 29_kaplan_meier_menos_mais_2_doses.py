...
# ─────────────────────────────────────────────────────────────────────────
# Curva de sobrevida (Kaplan-Meier) durante a internação, comparando quem
# tomou menos de 2 doses (0 ou 1) vs. mais de 2 doses (3 ou 4). Pacientes
# com exatamente 2 doses são excluídos (não se encaixam em nenhum grupo),
# mesma definição usada no forest plot 20_forest_plot_doses_menos_mais.py.
...
OUTPUT_PNG = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "kaplan_meier_menos_mais_2_doses.png")
...
# ════════════════════════════════════════════════════════════
# 2. CARREGAR DADOS E DEFINIR OS DOIS GRUPOS
# ════════════════════════════════════════════════════════════
dados_full = pd.read_excel(XLSX_PATH)
n_excluidos_2doses = int((dados_full["Vacinas"] == 2).sum())
dados = dados_full[dados_full["Vacinas"] != 2].copy()
dados["Grupo"] = np.where(dados["Vacinas"] < 2, "Menos de 2 doses",
                           "Mais de 2 doses")
...
grupos_info = {}
for grupo in ["Menos de 2 doses", "Mais de 2 doses"]:
...
sub_menos = dados[dados["Grupo"] == "Menos de 2 doses"]
sub_mais = dados[dados["Grupo"] == "Mais de 2 doses"]
chi2_lr = logrank_test(sub_menos["Dias_permanência"], sub_menos["Óbito"],
                        sub_mais["Dias_permanência"], sub_mais["Óbito"])
...
estilos = {
    "Menos de 2 doses": dict(color=COR_MENOS, lw=2.6, ls="-", zorder=4, alpha=0.95),
    "Mais de 2 doses": dict(color=COR_MAIS, lw=2.8, ls="-", zorder=5, alpha=0.95),
}

for grupo, estilo in estilos.items():
    km = grupos_info[grupo]["km"]
    ax.step(km["t"], km["S"], where="post", **estilo)
...
legend_elements = [
    Line2D([0], [0], color=COR_MAIS, lw=2.8,
           label=f"Mais de 2 doses (n={grupos_info['Mais de 2 doses']['n']}, "
                 f"óbitos={grupos_info['Mais de 2 doses']['obitos']})"),
    Line2D([0], [0], color=COR_MENOS, lw=2.6,
           label=f"Menos de 2 doses (n={grupos_info['Menos de 2 doses']['n']}, "
                 f"óbitos={grupos_info['Menos de 2 doses']['obitos']})"),
]
...
ax.text(0.99, 0.98, f"Log-rank (< 2 vs. > 2 doses)\nχ²={chi2_lr:.2f}, gl=1, "
                     f"{formata_p(p_lr)}",
...
fig.text(0.5, 0.985,
          "Sobrevida Durante a Internação: Menos de 2 vs. Mais de 2 Doses",
          ha="center", va="top", fontsize=17, fontweight="bold", color=TEXT)
fig.text(0.5, 0.945,
          f"n = {n_total} (excluídos {n_excluidos_2doses} com exatamente "
          f"2 doses) | Óbitos = {n_obitos} | Tempo = dias de permanência | "
...
print(f"n = {n_total} (excluídos {n_excluidos_2doses} com exatamente 2 doses)")
print(f"Log-rank < 2 vs. > 2 doses: chi2={chi2_lr:.3f}, p={p_lr:.4f}")
plt.show()
