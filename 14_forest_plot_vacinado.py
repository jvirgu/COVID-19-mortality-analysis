...
df_raw = pd.DataFrame([{
    "label": "Vaccinated", "n_geral": n_vacinado, "n_obito": n_vacinado_obito,
...
print(f"Overall n = {n_total} | Deaths n = {n_deaths} | Reference = Not vaccinated")
...
draw_panel(ax_or, "OR", "IC_inf", "IC_sup", "p_OR",
           "Crude OR (95% CI)", xlim=(0.3, 3))
draw_panel(ax_ora, "ORa", "ICa_inf", "ICa_sup", "p_ORa",
           "Adjusted OR (95% CI)", xlim=(0.3, 3))
...
# ── Main title ────────────────────────────────────────────────────────────
fig.text(0.50, 1.14,
         "Forest Plot — Vaccination Status vs. In-Hospital Death",
         ha="center", va="top",
         fontsize=30, fontweight="bold", color=TEXT)

# ── Dynamic subtitle ────────────────────────────────────────────────────
subtitle = (f"Reference category: not vaccinated | "
