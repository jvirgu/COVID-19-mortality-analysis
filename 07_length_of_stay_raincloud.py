...
# Overall (derived from the two groups)
n_overall = 703
med_overall = 4  # approximate weighted median
...
# Overall SD via the pooled formula
...
print(f"Overall: mean={mean_overall}, SD={dp_overall}")
...
data_overall = np.concatenate([data_alta, data_obito])  # combined real data

# ════════════════════════════════════════════════════════════
# 3. PALETTE AND LAYOUT
# ════════════════════════════════════════════════════════════
BG = "#FFFFFF"
PANEL = "#F6F8FA"
BORDER = "#D0D7DE"
TEXT = "#1F2328"
SUBTEXT = "#57606A"
COR_OV = "#7C6FC0"   # purple — Overall
COR_ALT = "#1D9E75"  # green — Discharge
COR_OBT = "#E07B39"  # orange — Death

# Y positions: Overall=2, Death=1, Discharge=0
POS = {"Overall": 2.0, "Death": 1.0, "Discharge": 0.0}
groups = [
    # (label, data, color, n, med, ic_lo, ic_hi, p25, p75, mean, sd)
...
# ════════════════════════════════════════════════════════════
# 5. MEDIAN DELTA — arrow between Death and Discharge
# ════════════════════════════════════════════════════════════
delta_md = med_obt - med_alta  # 5 - 3 = 2
x_arrow = max(max_alta, max_obt) * 0.51  # arrow X position
by_disc = POS["Discharge"] - 0.08
by_death = POS["Death"] - 0.08

# vertical line connecting the two medians
ax.annotate("",
            xy=(x_arrow, by_death),
...
# ════════════════════════════════════════════════════════════
# 6. NEGATIVE BINOMIAL MODEL ANNOTATION
# ════════════════════════════════════════════════════════════
ax.text(0.98, 0.04,
