...
def read_word_table(file_path, table_index=0):
    """
    This table has 1 merged header row (row 0).
    Data starts on row 1.
    Columns: label | n_overall | n_death | OR_CI | p_OR | ORa_CI | p_ORa
    """
...
def parse_or_ci(text):
    """
    Parses strings like '4.23 (3.02-5.91)' or '21.48 (6.54-70.49)'.
    ...
    if match:
        try:
            ...
            # discard non-estimable cases (OR ~ 0 or infinite CI)
...
# This table has no group headers or reference rows — every row is a variable
df_raw["is_ref"] = False
