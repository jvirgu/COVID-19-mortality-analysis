...
# ════════════════════════════════════════════════════════════
# 2. GRÁFICO EM PAINÉIS (pequenos múltiplos) — um por grupo de vacinação
# ════════════════════════════════════════════════════════════
grupos = sorted(GRUPO_LABELS)
fig, axes = plt.subplots(1, len(grupos), figsize=(12, 6), facecolor=BG,
                          sharey=True)

for ax, grupo in zip(axes, grupos):
    sub = dados[dados["Vacinado"] == grupo]
    tab = sub.groupby(["Ano", "Óbito"]).size().unstack(fill_value=0)
    tab = tab.reindex(anos_todos, fill_value=0)
    anos_com_dados = sub["Ano"].unique()
    tab = tab.loc[tab.index.isin(anos_com_dados) | (tab.sum(axis=1) > 0)]
    ...
    ax.set_title(f"{GRUPO_LABELS[grupo]} (n={len(sub)})", fontsize=14,
...
fig.legend(handles, legend_labels, fontsize=11, frameon=True, edgecolor=BORDER,
           ...
           bbox_to_anchor=(0.5, 0.90), framealpha=0.97)

fig.text(0.5, 1.0, "Altas e Óbitos por Ano — Vacinados vs. Não Vacinados",
