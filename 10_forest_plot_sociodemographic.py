...
# Labels (explicit Arial font in each ax.text)
for i, row in df_raw.iterrows():
    lbl = row["label"]
    if row["is_header"]:
...
def draw_panel(ax, col_or, col_lo, col_hi, col_p, title, xlim):
    ...
    # X-axis tick labels — tick_params does not accept fontfamily
...
draw_panel(ax_or, "OR", "IC_inf", "IC_sup", "p_OR",
           "Crude OR (95% CI)", xlim=(0.05, 30))
draw_panel(ax_ora, "ORa", "ICa_inf", "ICa_sup", "p_ORa",
           "Adjusted OR (95% CI)", xlim=(0.05, 800))
