...
# ─────────────────────────────────────────────────────────────────────────
# Forest plot comparando quem tomou menos de 2 doses (0 ou 1) vs. mais de
# 2 doses (3 ou 4). Pacientes com exatamente 2 doses são excluídos, pois
# não se encaixam em nenhum dos dois grupos.
# ─────────────────────────────────────────────────────────────────────────
XLSX_PATH = "703pacientes.xlsx"
OUTPUT_PNG = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "forest_doses_menos_mais_de_2.png")
...
# ════════════════════════════════════════════════════════════
# 1. CARREGAR DADOS E DEFINIR EXPOSIÇÃO (referência = < 2 doses)
# ════════════════════════════════════════════════════════════
dados_todos = pd.read_excel(XLSX_PATH)
dados_todos["MaisDe2"] = (dados_todos["Vacinas"] > 2).astype(int)
n_excluidos_2doses = int((dados_todos["Vacinas"] == 2).sum())

dados = dados_todos[dados_todos["Vacinas"] != 2].copy()

n_total_completo = len(dados_todos)
n_analisado = len(dados)
n_obitos = int(dados["Óbito"].sum())
n_mais = int(dados["MaisDe2"].sum())
n_mais_obito = int(dados.loc[dados["MaisDe2"] == 1, "Óbito"].sum())
n_menos = n_analisado - n_mais
n_menos_obito = n_obitos - n_mais_obito

# Teste qui-quadrado (< 2 doses vs. > 2 doses)
tab_chi2 = pd.crosstab(dados["MaisDe2"], dados["Óbito"])
chi2, p_chi2, dof_chi2, _ = chi2_contingency(tab_chi2)
...
OR, lo, hi, p = fit_or("Óbito ~ MaisDe2", "MaisDe2", dados)
formula_adj = "Óbito ~ MaisDe2 + " + " + ".join(ADJUST_VARS)
ORa, loa, hia, pa = fit_or(formula_adj, "MaisDe2", dados)

df_raw = pd.DataFrame([{
    "label": "Mais de 2 doses (3 ou 4)", "n_geral": n_mais,
    "n_obito": n_mais_obito, "OR": OR, "IC_inf": lo, "IC_sup": hi,
...
print(f"n total = {n_total_completo} | analisados = {n_analisado} "
      f"(excluídos {n_excluidos_2doses} com exatamente 2 doses) | "
      f"Óbitos = {n_obitos}")
print(f"< 2 doses: n={n_menos}, óbitos={n_menos_obito} | "
      f"> 2 doses: n={n_mais}, óbitos={n_mais_obito}")
print(f"Qui-quadrado: chi2={chi2:.3f}, gl={dof_chi2}, p={p_chi2:.4f}")
...
fig.text(0.50, 1.16,
          "Forest Plot — Menos de 2 Doses vs. Mais de 2 Doses",
          ha="center", va="top", fontsize=30, fontweight="bold", color=TEXT)
subtitle = (f"Referência: < 2 doses (n={n_menos}, óbitos={n_menos_obito}) | "
            f"Exposição: > 2 doses (n={n_mais}, óbitos={n_mais_obito}) | "
            f"excluídos {n_excluidos_2doses} com exatamente 2 doses")
fig.text(0.50, 1.06, subtitle, ha="center", va="top", fontsize=15, color=SUBTEXT)
fig.text(0.50, 1.005,
          f"χ² = {chi2:.2f}, gl={dof_chi2}, {formata_p(p_chi2)}",
          ha="center", va="top", fontsize=15, color=SUBTEXT)
...
fig.text(0.03, -0.16,
          ...
          "de Confiança de 95% | χ² = teste qui-quadrado de associação | "
          "Pacientes com exatamente 2 doses foram excluídos por não se "
          "encaixarem em nenhum dos dois grupos | Ajustado por sexo, "
          "comorbidades, idade, estado civil, escolaridade e tempo de "
          "internação",
          color=SUBTEXT, fontsize=11.5, style="italic")

plt.tight_layout(rect=[0, 0.03, 1, 1.90])
