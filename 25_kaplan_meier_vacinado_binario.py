...
# vacinados (qualquer nº de doses) vs. não vacinados. Tempo = dias de
# permanência hospitalar; evento = óbito (alta é tratada como censura).
# ─────────────────────────────────────────────────────────────────────────
XLSX_PATH = "703pacientes.xlsx"
OUTPUT_PNG = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "kaplan_meier_vacinado_binario.png")
...
COR_VAC = "#1D9E75"      # vacinados (protetor, em destaque)
...
# ════════════════════════════════════════════════════════════
# 2. CARREGAR DADOS E DEFINIR OS DOIS GRUPOS (vacinado, independente da dose)
# ════════════════════════════════════════════════════════════
...
sub_vac = dados[dados["Grupo"] == "Vacinados"]
chi2_lr = logrank_test(sub_nao_vac["Dias_permanência"], sub_nao_vac["Óbito"],
                        sub_vac["Dias_permanência"], sub_vac["Óbito"])
...
estilos = {
    ...
    "Vacinados": dict(color=COR_VAC, lw=2.8, ls="-", zorder=5, alpha=0.95),
}
...
legend_elements = [
    Line2D([0], [0], color=COR_VAC, lw=2.8,
           label=f"Vacinados (n={grupos_info['Vacinados']['n']}, "
                 f"óbitos={grupos_info['Vacinados']['obitos']})"),
    Line2D([0], [0], color=COR_NAO_VAC, lw=2.6,
           label=f"Não vacinados (n={grupos_info['Não vacinados']['n']}, "
...
# Anotação do teste de log-rank
ax.text(0.99, 0.98, f"Log-rank (vacinados vs. não vacinados)\n"
...
print(f"Log-rank vacinados vs. não vacinados: chi2={chi2_lr:.3f}, p={p_lr:.4f}")
plt.show()
