...
# vacina (0 a 4). Tempo = dias de permanência hospitalar; evento = óbito
# (alta é tratada como censura). Teste de log-rank multivariado (k-amostras)
# para associação global entre nº de doses e sobrevida.
# ─────────────────────────────────────────────────────────────────────────
XLSX_PATH = "703pacientes.xlsx"
OUTPUT_PNG = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "kaplan_meier_por_dose.png")
...
fig.text(0.5, 0.985, "Sobrevida Durante a Internação por Número de Doses de Vacina",
          ha="center", va="top", fontsize=17.5, fontweight="bold", color=TEXT)
fig.text(0.5, 0.945,
