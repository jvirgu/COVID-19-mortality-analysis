...
# vacina, EXCLUINDO:
#   (1) pacientes internados em 2020 — ano anterior ao início da vacinação,
#       em que todos são necessariamente "0 doses";
#   (2) pacientes com exatamente 1 dose — grupo pequeno e sujeito a viés de
#       indicação (vacinação tardia/emergencial durante a internação), já
#       identificado como outlier nas análises anteriores.
# Restam os grupos 0, 2, 3 e 4 doses.
...
OUTPUT_PNG = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "kaplan_meier_por_dose_sem_2020_sem_1dose.png")
...
# ════════════════════════════════════════════════════════════
# 1. CARREGAR DADOS (excluindo 2020 e pacientes com 1 dose)
# ════════════════════════════════════════════════════════════
...
n_excluidos_1dose = int((dados_full["Vacinas"] == 1).sum())
dados = dados_full[(dados_full["Ano"] != 2020) &
                    (dados_full["Vacinas"] != 1)].copy()
...
ax.text(0.99, 0.98,
        f"Log-rank multivariado ({'/'.join(DOSE_LABELS[d] for d in doses)})\n"
        f"χ²={chi2_lr:.2f}, gl={gl_lr}, {formata_p(p_lr)}",
...
fig.text(0.5, 0.985,
          ...
          "(excluindo 2020 e pacientes com 1 dose)",
          ha="center", va="top", fontsize=16.5, fontweight="bold", color=TEXT)
fig.text(0.5, 0.90,
          f"n = {n_total} (excluídos {n_excluidos_2020} de 2020 e "
          f"{n_excluidos_1dose} com 1 dose) | Óbitos = {n_obitos} | "
...
print(f"n = {n_total} (excluídos {n_excluidos_2020} de 2020 e "
      f"{n_excluidos_1dose} com 1 dose)")
print(f"Log-rank multivariado ({'/'.join(DOSE_LABELS[d] for d in doses)}): "
      f"chi2={chi2_lr:.3f}, gl={gl_lr}, p={p_lr:.4f}")
plt.show()
